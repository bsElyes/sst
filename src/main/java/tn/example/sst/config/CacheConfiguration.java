...
import tn.example.sst.utils.PrefixedKeyGenerator;
...
public class CacheConfiguration {
    ...
    public HazelcastInstance hazelcastInstance(ApplicationProperties applicationProperties) {
        ...
        HazelcastInstance hazelCastInstance = Hazelcast.getHazelcastInstanceByName("sst-cache");
        if (hazelCastInstance != null) {
            log.debug("Hazelcast already initialized");
            return hazelCastInstance;
        }
        Config config = new Config();
        config.setInstanceName("sst-cache");
        ...
        config.addMapConfig(initializeDefaultMapConfig(applicationProperties));
        config.addMapConfig(initializeDomainMapConfig(applicationProperties));
    ...
    private MapConfig initializeDomainMapConfig(ApplicationProperties applicationProperties) {
        MapConfig mapConfig = new MapConfig("tn.example.sst.domain.*");
